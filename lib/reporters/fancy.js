 ...
 *
 * @example
 * formatIntro({ count: 1, description: "Matching XX glob pattern" })
 * // => "Running 1 file through tsd-lite"
 * // => "Matching XX glob pattern"
 *
 * @example
 * formatIntro({ count: 4, description: "Matching XX glob pattern" })
...
const formatIntro = ({ count, description }) => {
  ...
  return [`Running ${count} test ${files} through tsd-lite`, description, ""]
    .map(bold)
    .join("\n")
}
